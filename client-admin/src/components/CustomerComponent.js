...
class Customer extends Component {
    ...
    const customers = this.state.customers.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trCustomerClick(item)}>
          ...
          <td>
            {item.active === 0 ?
              <span className="link-email" onClick={() => this.lnkEmailClick(item)}>EMAIL</span>
              :
              <span className="link-deactive" onClick={() => this.lnkDeactiveClick(item)}>DEACTIVE</span>}
          </td>
        </tr>
      );
    });
  ...
  lnkDeactiveClick(item) {
    if (window.confirm("ARE YOU SURE DEACTIVE?")) {
